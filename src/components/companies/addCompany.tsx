...
import { addCompanyThunk } from "@/src/store/slices/companySlice";
...
const AddCompanyModal: React.FC<AddCompanyModalProps> = ({
  ...
  const handleSubmit = async (values: Record<string, unknown>) => {
    try {
      ...
      await dispatch(addCompanyThunk(payload)).unwrap();
      dispatch(
        showAlert({
      ...
      const errorMessage =
        typeof error === "string" ? error : t("company.createFailed");
      dispatch(
        showAlert({
